...
for(index in 1:3){
  ...
  for(ratio in c(1.1, 1.5, 2)){
    n <- p * ratio
    c0 <- sqrt(log(p)/n)
    load(file=paste0("Simulation_results/Non_Gaussian_t_distribution_",nu[index],"_global_null_p_", p, "_n_", n, "_c0_", c0, ".RData"))
  ...
  plot_list[[index]][[1]] <- base_plot + geom_qq(distribution = qunif,
    ...
    ggtitle(bquote(nu~"="~.(nu[index])))+ theme(text = element_text(size = 30)) + 
