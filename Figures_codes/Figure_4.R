...
plot_data_melt <- plot_data_melt[plot_data_melt$Select.Prob < 0.9,]
...
p1 <- ggplot(plot_data_a, aes(Effect, pvalue, color=Method, linetype = Inference)) +
  ...
  xlab("Effect Size (\u0394)") + ylab("Power at level \u03b1 = 0.05") + labs(color='') + labs(linetype='') +
...
p2 <- ggplot(plot_data_b, aes(Effect, pvalue, color=Method, linetype = Inference)) +
  stat_summary_bin(fun = "power", geom = "line", orientation = 'x', breaks = seq(0, 1, 0.1)) + 
  stat_summary_bin(fun = "power", geom = "point", orientation = 'x',breaks = seq(0, 1, 0.1)) +
  xlab("Effect Size (\u0394)") + ylab("Power at level \u03b1 = 0.05") + labs(color='') + labs(linetype='') +
