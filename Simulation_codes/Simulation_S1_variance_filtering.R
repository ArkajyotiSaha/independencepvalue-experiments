...
#Code for simulation under global null after variance filtering. 
...
eval_total <- function(i0, p, n, c0) {
  ...
  X <- X[,orderedcolVar_X[1:(p/5)]]#variance filtering, keep the variables with highest variance
...
for(ratio in c(1.1, 1.5, 2)){
  ...
  save(result, file = paste0("Simulation_results/Variable_selection_Variance_filtering_global_null_p_", p, "_n_", n, "_c0_", c0, ".RData"))
}
