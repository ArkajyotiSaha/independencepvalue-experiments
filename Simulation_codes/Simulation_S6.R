...
#Code for simulation under alternative to check what effect does conditioning have on power. 
#input: i0 = Seed, p = # of variables, n = # of samples,.
#output: vector of our proposed selective p-value and the selective p-value where we only condition on the diagonal blocks, effect size, and zeta (as defined in the Supplementary Materials S11.3.1)
eval_total <- function(i0, p, n) {
  ...
  Sigma <- Sigma_cor#generate a dense popoulation covaiance matrix as in Simulaiton_2.R
  ...
  c0 <- 1 - clust_result$height[p - 1] + sqrt(log(p)/n)#determione the threshold as in Simulaiton_2.R
  ...
  if(length(unique(block_diag_structure))> 1){
    ...
    t0 <- mean(test_hyp$statistic >= sip[1, ][sip[2, ] == TRUE])#selective p-value when we only condition on the diagonal blocks
    set.seed(i0)
    t1 <- independencepvalue::selective_p_val(S = cov(X), n = n, CP = block_diag_structure, c = c0, k = k1, d0 = 1, mc_iter = 1000)#our proposed selective p-value
    t2 <- effect_size(Sigma, CP = block_diag_structure, k = k1)#effect size
    t3 <- min(svd(test_stat_CCA_new(cov(X), block_diag_structure, k1)$S12w)$u^2)#zeta, as defined in the Supplementary Materials S11.3.1
