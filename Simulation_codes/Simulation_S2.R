...
#Code for simulation under global null with a given threshold for data with t distribution.
#input: i0 = Seed, p = # of variables, n = # of samples, c0 = threshold, nu = degrees of freedom of the t-distribution. 
#output: A vector of seletive and classical p-values.
eval_total <- function(i0, p, n, c0, nu) {
  set.seed(i0)
  X <- matrix(rt(n * p, df = nu), n, p)#generate data from t distribution under global null
