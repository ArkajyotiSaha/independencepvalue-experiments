...
Coeffmatrix<-expand.grid(network=c(3, 4), ratio=c(1.2, 1.4, 1.5, 2), sigma=c(0, 0.5, 3), starting=seq(1, 10000, by=1000))



p <- 100
network <- Coeffmatrix$network[index]
ratio <- Coeffmatrix$ratio[index]
n <- p * ratio
sigma <- Coeffmatrix$sigma[index]
starting <- Coeffmatrix$starting[index]


fip <- data.table::fread(paste0("DREAM5_data/gold_standard_edges_only/DREAM5_NetworkInference_Edges_Network", network, ".tsv"))
dip <- data.table::fread(paste0("DREAM5_data/Training/net", network, "_expression_data.tsv"))
